package org.com.steris.com.steris
...
object Iot_kafka_Consumer {  

 def main(args:Array[String])
  {
    
   val sparkConf = new SparkConf().setAppName("SterisIotAnalytics").setMaster("local[*]").set("spark.sql.crossJoin.enabled", "true")          
...
//            sourcelog => {
//               sourcelog.collect().toBuffer.foreach(
//                   (x: (Any, String)) => {
//                     val inlog = x._2.split(' ').map(x => x.toDouble).tail
//                     println(model.predict(Vectors.dense(inlog)))
//                   }
//            }
//        )        
